...
import ro.h23.dars.webscraper.exception.ExternalServiceException;
...
public class GenericExternalApiService {
    ...
    public <T> T sendRequest(HttpMethod httpMethod, String uriString, Object bodyObject, Class<T> clasz, boolean isAuthorization) {
        ...
        MultipartBodyBuilder multipartBodyBuilder = null;
        if (bodyObject instanceof MultipartBodyBuilder) {
            contentType = MediaType.MULTIPART_FORM_DATA_VALUE;
            multipartBodyBuilder = (MultipartBodyBuilder) bodyObject;
        } else if (bodyObject instanceof byte[]) {
            contentType = MediaType.APPLICATION_OCTET_STREAM_VALUE;
        }
        if (isAuthorization || authenticationPath == null || authToken != null) {
            ...
            if (bodyObject != null) {
                RequestHeadersSpec<?> headerSpec = null;
                if (multipartBodyBuilder != null) {
                    headerSpec = bodySpec.body(BodyInserters.fromMultipartData(multipartBodyBuilder.build()));
                } else {
                    headerSpec = bodySpec.bodyValue(bodyObject);
                }
            ...
            try {
                ...
                } else if (authenticationPath == null) {
                    throw new ExternalServiceException("Authorization is required", e);
                }
